...
def _capabilities() -> Capabilities:
    ...
    return Capabilities(
        ...
        debug={
            ...
            "_all_relevant_env": ", ".join(f"{k}={v[:6]}***" if "KEY" in k.upper() and v else f"{k}={v}" for k, v in sorted(os.environ.items()) if any(p in k.upper() for p in ("UIDESIGN", "AI_", "DEEPSEEK", "OPENAI", "TESSERACT"))),
        },
    )
